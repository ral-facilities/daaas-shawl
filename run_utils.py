...
def run_term_rsync_up(hostname, username, password, local_path, remote_path):
    """Run command remotely, opening a local terminal."""
    term_cmd = f"sshpass -p {shlex.quote(password)} rsync -e 'ssh -oUserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no' --stats --progress {shlex.quote(local_path)}/ -aXxv {shlex.quote(username)}@{shlex.quote(hostname)}:{shlex.quote(remote_path)} ; read -n 1 -p 'Command finished. Press any key to continue.'"
    logging.info(term_cmd)
    subprocess.Popen([term, "-geometry", "100x32", "-e", term_cmd])
...
def run_term_rsync_down(hostname, username, password, local_path, remote_path):
    """Run command remotely, opening a local terminal."""
    term_cmd = f"sshpass -p {shlex.quote(password)} rsync -e 'ssh -oUserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no' -aXxv --stats --progress {shlex.quote(username)}@{shlex.quote(hostname)}:{shlex.quote(remote_path)}/ {shlex.quote(local_path)} ; read -n 1 -p 'Command finished. Press any key to continue.'"
    logging.info(term_cmd)
    subprocess.Popen([term, "-geometry", "100x32", "-e", term_cmd])
